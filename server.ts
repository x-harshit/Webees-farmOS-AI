...
// Serve zip file explicitly with proper attachment disposition
app.get(['/netlify-deploy.zip', '/public/netlify-deploy.zip'], (req, res) => {
  const zipPath = path.join(__dirname, 'public', 'netlify-deploy.zip');
...
async function callGeminiWithFallback(ai: GoogleGenAI, contents: any[]) {
  // Use model aliases per official @google/genai guidelines
...
Return a valid JSON object with the following exact keys:
{
  ...
  "outbreakForecast": [
    ...
    // Provide clean fallback diagnosis if API quota is reached so UI does not fail
    res.json({
      isHealthy: false,
      crop: req.body?.crop || 'Plant Specimen',
